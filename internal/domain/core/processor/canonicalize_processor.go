...
func NewCanonicalizeProcessor(counter core.TokenCounter) *CanonicalizeProcessor {
	return &CanonicalizeProcessor{counter: counter}
}
...
func (p *CanonicalizeProcessor) Process(_ context.Context, req *core.RefineRequest) (*core.RefineRequest, core.ProcessResult, error) {
	...
	systemMessages, memoryMessages, activeMessages := core.StablePromptSegments(updated.Messages)
	updated.Messages = append(append(systemMessages, memoryMessages...), activeMessages...)
	updated.RAGChunks = core.StableRAGChunks(updated.RAGChunks)
	for i, chunk := range updated.RAGChunks {
		updated.RAGChunks[i].Sources = core.StableSources(chunk.Sources, chunk.Source)
	}
