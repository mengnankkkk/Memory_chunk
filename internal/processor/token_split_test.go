...
func (r runeCounter) CountFragment(fragment engine.RAGFragment) int {
	return r.CountText(fragment.Content)
}

func (r runeCounter) CountChunk(chunk engine.RAGChunk) int {
	return r.CountText(engine.ChunkText(chunk))
}

func (r runeCounter) CountRequest(req *engine.RefineRequest) int {
	return r.CountText(req.OptimizedPrompt)
}

func TestSplitFragmentByTokenBudgetPreservesMetadata(t *testing.T) {
	counter := runeCounter{}
	fragment := engine.RAGFragment{
		Type:     engine.FragmentTypeCode,
		Content:  "abcd\nefgh\nijkl",
		Language: "go",
	}
