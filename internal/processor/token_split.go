...
func splitTextByTokenBudget(counter engine.TokenCounter, text string, maxTokens int) []string {
...
func splitLinesByTokenBudget(counter engine.TokenCounter, lines []string, maxTokens int) []string {
	parts := make([]string, 0)
...
func splitRunesByTokenBudget(counter engine.TokenCounter, text string, maxTokens int) []string {
	if maxTokens <= 0 || text == "" {
		return []string{text}
	}
	runes := []rune(text)
...
func splitFragmentByTokenBudget(counter engine.TokenCounter, fragment engine.RAGFragment, maxTokens int) []engine.RAGFragment {
