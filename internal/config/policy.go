...
func LoadPolicies(path string) (map[string]engine.RuntimePolicy, error) {
	...
	items := make(map[string]engine.RuntimePolicy, len(file.Policies))
	for _, policy := range file.Policies {
		items[policy.Name] = engine.RuntimePolicy{
			Name:        policy.Name,
			BudgetRatio: policy.BudgetRatio,
			Steps:       append([]string(nil), policy.Steps...),
			Snip: engine.SnipConfig{
