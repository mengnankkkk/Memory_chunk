...
func (c *Counter) CountFragment(fragment core.RAGFragment) int {
	return c.CountText(core.FragmentText(fragment))
}

func (c *Counter) CountChunk(chunk core.RAGChunk) int {
	return c.CountText(core.ChunkText(chunk))
}

func (c *Counter) CountRequest(req *core.RefineRequest) int {
	if req.OptimizedPrompt != "" {
		return c.CountText(req.OptimizedPrompt)
	}
	return c.CountText(core.AssemblePrompt(req))
}
